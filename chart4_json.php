...
$sql="SELECT ROUND(damage_dealt/battles_count,1) as dmg,date,time FROM `player` where idp='$idac' and date>='$date2' order by date,time";
$result = mysql_query( $sql,$connect ) or die("<br>Couldn t execute query.".mysql_error()); 
	while($row = mysql_fetch_assoc($result)) {
		...
		$data[] = array($row['date']." ".$row['time'],(float)$row['dmg']);
	}
