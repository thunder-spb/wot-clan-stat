...
$result = mysql_query("SELECT count(*) as cnt FROM `cat_tanks` where level=10 or (level=9 and (class='mediumTank' or class='AT-SPG')) or (level=8 and class='SPG')");
$row = mysql_fetch_array($result,MYSQL_ASSOC); 
$count_t = $row['cnt']; 
//$message="<table  border='1' width='900px'>";
$sql="SELECT * FROM `cat_tanks` where ((level=8 and (class='heavyTank' or class='mediumTank' or class='AT-SPG' or class='lightTank')) or (level=6 and class='SPG')) ORDER BY level desc,class,nation";
