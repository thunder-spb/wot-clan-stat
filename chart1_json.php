...
$sql="SELECT ROUND(wins*100/battles_count,2) as proc,date,time FROM `player` where idp='$idac' and date>='$date2' order by date,time";
$result = mysql_query( $sql,$connect ) or die("<br>Couldn t execute query.".mysql_error()); 
	while($row = mysql_fetch_assoc($result)) {
		...
		$data[] = array($row['date']." ".$row['time'],(float)$row['proc']);
	}
