...
$sql="SELECT a.localized_name ,max(c.battle_count) as battle_count,max(c.master) as mst,ROUND((max(c.win_count)*100/max(c.battle_count)),2) as proc,a.level,a.class as cls,a.nation from `player_btl` c, cat_tanks a where idp='$idac' and c.idt=a.wotidt group by c.idt ORDER BY ".$sidx." ".$sord." LIMIT ".$start.", ".$limit;
...
$data=new stdclass;
	...
	while($row = mysql_fetch_assoc($result)) {
		...
		$msg="--";
		if ($mst<>0){
			$msg='<img src="images/MarkOfMastery'.$mst.'.png" style="width: 20px; height:20px;" align="absmiddle"/>';
		}
		$data->rows[$i]['cell'] = array($row['localized_name'],$msg,$row['battle_count'],$row['proc'],$row['level'],$row['cls'],$row['nation']);
		$i++;
	}
